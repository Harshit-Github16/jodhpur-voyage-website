  ...
  useEffect(() => {
    ...
    const fetchTeam = async () => {
      try {
        ...
        if (list.length) setTeamMembers(list);
      } catch (err) {
        console.warn('Failed to load team', err.message || err);
      }
    };
    fetchTeam();
        ...
        <div className="container">
          ...
          <div className="team-grid">
            {teamMembers && teamMembers.length > 0 ? (
              teamMembers.map((m) => (
                <div className="team-member-card" key={m._id || m.id || m.slug || m.name}>
                  <div className="member-header">
                    <div className="member-avatar-wrap">
                      <img
                        src={m.image || m.photo || '/images/image-12.jpg'}
                        alt={m.name || m.title || 'Team member'}
                        className="member-avatar-img"
                        onError={(e) => { e.target.src = '/images/image-12.jpg'; }}
                      />
                    </div>
                    <div className="member-info">
                      <h3 className="member-name">{m.name || m.title}</h3>
                      <div className="member-role">{m.role || m.position || ''}</div>
                      <span className="member-specialty"><i className="fas fa-map-marker-alt"></i> {m.specialty || m.expertise || ''}</span>
                    </div>
                  </div>
                  <div className="member-bio">
                    <p>{m.bio || m.description || ''}</p>
                    {m.quote ? <div className="member-quote">{m.quote}</div> : null}
                  </div>
                </div>
              ))
            ) : (
              // fallback static members preserved
              <>
                {/* MEMBER 1: VIKEY */}
